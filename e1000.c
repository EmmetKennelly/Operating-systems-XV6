...
void parse1(char *packet, int len){
    ...
    struct tcp_h * tcp;
        ...
        printf("\n");
		   ...
		   short srcPort = *(( short*)tcp->srcPort);	
		   srcPort = ((srcPort >8 )|(srcPort <8));
		   printf("%u"+"\n",srcPort);

		   printf("Destination port is - ");
		   short destPort = *((short*)tcp->destPort);
		   destPort = ((destPort >8) | (destPort <8));
		   printf("%u"+"\n",destPort);
		   
		   int size = v_ihl.len;
		   if( size >= sizeof(struct ethernet_h) + sizeof(struct ip_h) + sizeof(struct tcp_h)+ sizeof(struct tls_h)){ 	
			tls = (struct tls_h *) (packet + sizeof(struct ethernet_h) + sizeof(struct ip_h) + sizeof(struct tcp_h)); 
			char versionFirst = *((char*)tls->version);
			char versionLast = *(( har*)tls->version+1);
			if (versionFirst == 0x03 && versionLast == 0x01){
				printf("Yes : \n");
			}
			else{
				printf("No : \n");
